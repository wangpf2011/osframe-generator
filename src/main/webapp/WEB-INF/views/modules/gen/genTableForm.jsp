...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/gen/genTable/form?id=${lnGenTable.id}&name=${lnGenTable.name}">业务表<shiro:hasPermission name="gen:genTable:edit">${not empty lnGenTable.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="gen:genTable:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<c:choose>
		<c:when test="${empty lnGenTable.name}">
			<form:form id="inputForm" modelAttribute="lnGenTable" action="${ctx}/gen/genTable/form" method="post" class="form-horizontal">
				<form:hidden path="id"/>
		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="lnGenTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				<form:hidden path="id"/>
				<tags:message content="${message}"/>
				<fieldset>
					<legend>基本信息</legend>
					...
					<div class="control-group">
						...
						<div class="controls">
							...
							<form:select path="parentTableFk" cssClass="input-xlarge">
								<form:option value="">无</form:option>
								<form:options items="${lnGenTable.columnList}" itemLabel="nameAndComments" itemValue="name" htmlEscape="false"/>
							</form:select>
					...
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							...
							<tbody>
							<c:forEach items="${lnGenTable.columnList}" var="column" varStatus="vs">
